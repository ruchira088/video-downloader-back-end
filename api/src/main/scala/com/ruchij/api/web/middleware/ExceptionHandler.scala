...
import org.http4s.{ContextRequest, MessageFailure, Request, Response, Status}

object ExceptionHandler {
  private val logger = Logger[ExceptionHandler.type]

  def apply[F[_]: Sync, A](
    contextHttpApp: Kleisli[F, ContextRequest[F, A], Response[F]]
    ...
    Kleisli[F, ContextRequest[F, A], Response[F]] { contextRequest =>
      Sync[F].handleErrorWith(contextHttpApp.run(contextRequest)) { throwable =>
        entityResponseGenerator[F](throwable)(throwableResponseBody(throwable))
          ...
          .flatTap(logErrors[F](throwable, contextRequest.req))
      }
    }

  private def logErrors[F[_]: Sync](throwable: Throwable, request: Request[F])(response: Response[F]): F[Unit] =
    if (response.status >= Status.InternalServerError)
      logger.error[F](
        s"Service error response with ${response.status} status code returned. Request ${request.method} ${request.uri}",
        throwable
      )
  ...
  private val throwableStatusMapper: Throwable => Status = {
    ...
    case _: DecodingFailure | _: IllegalArgumentException | _: MessageFailure | _: ValidationException |
        _: UnsupportedVideoUrlException =>
      Status.BadRequest
  ...
  private val throwableResponseBody: Throwable => ErrorResponse = {
    ...
    case throwable =>
      Option(throwable.getCause).fold(ErrorResponse(NonEmptyList.one(throwable.getMessage)))(throwableResponseBody)
  }
