...
class KeySpacedKeyValueStore[F[_]: Functor, K <: KVStoreKey : KeySpacedKeyEncoder[F, *], V: KVCodec[F, *]](
  ...
  def put(key: K, value: V): F[Unit] = keyValueStore.put[K, V](key, value, key.maybeTtl.orElse(keySpace.maybeTtl)).void

  def remove(key: K): F[Unit] = keyValueStore.remove(key).void
}
