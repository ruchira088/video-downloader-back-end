...
class TestCognitoUserService(unittest.TestCase):
    def setUp(self):
        ...
        user_pool_client_creation_response = cognito_client.create_user_pool_client(
            ...
            ClientName=f'{__name__}-api-client'
        )

        user_pool_client_id = user_pool_client_creation_response['UserPoolClient']['ClientId']
        self.user_validation_service: Union[UserValidationService, MagicMock] = MagicMock()
    ...
    def test_create_user(self):
        ...
        created_user: User = self.user_service.create_user(email=sample_user.email, password=sample_password)
